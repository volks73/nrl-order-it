 ...
 *
 * ***** END LICENSE BLOCK ***** */
var EXPORTED_SYMBOLS = [ "mil.navy.nrl.NRLOrderIt.SQL.JobOrderNumber" ];
...
/**
 * The <code>JobOrderNumber</code> module.
 */
mil.navy.nrl.NRLOrderIt.SQL.JobOrderNumber = ( function () {
	
	/*
	 * Private variables.
	 */
	
	var _logger = Log4Moz.repository.getLogger("mil.navy.nrl.NRLOrderIt.SQL.JobOrderNumber");
	var _NRLOrderIt = mil.navy.nrl.NRLOrderIt;
	var _SQL = _NRLOrderIt.SQL;
	var _conn = null;
	var _sqlTable = {
			name : 'job_order_numbers',
			columns : [
			           {
	...
	return {
		...
		add : function (aJobOrderNumber, aCallback) {
			_logger.trace("add opened");
			
			aJobOrderNumber.dateAdded = new Date();
			aJobOrderNumber.dateRemoved = null;
			...
			for ( let i = _SQL.ID_INDEX + 1; i < columns.length; i += 1 ) {
				let value = aJobOrderNumber[columns[i].property];
									...
									handleCompletion : function (aReason) {
										...
										if ( aReason === Components.interfaces.mozIStorageStatementCallback.REASON_FINISHED ) {
											...
											if ( typeof(aCallback) !== 'undefined') {
												aJobOrderNumber.id = lastInsertId;
		...
		edit : function (aJobOrderNumber, aCallback) {
				...
				statement.executeAsync( {
					...
					handleCompletion : function (aReason) {
						...
						if ( aReason === Components.interfaces.mozIStorageStatementCallback.REASON_FINISHED ) {
							...
							if ( typeof(aCallback) !== 'undefined') {
								aCallback.completed(aJobOrderNumber);
							}
						}
						else {
		...
		remove : function (aJobOrderNumber, aCallback) {
			_logger.trace("remove opened");
			
			aJobOrderNumber.dateRemoved = new Date();
			...
			statement.params[_SQL.DATE_REMOVED_PARAM] = _SQL.convertDateToInteger(aJobOrderNumber.dateRemoved);
			statement.params[idColumn.param] = aJobOrderNumber[idColumn.property];
			...
			try {
				statement.executeAsync( {
					handleResult : function (aResultSet) {
						...
						{  
							jobOrderNumbers[rowCount] = {};
							
							for ( let i = 0; i < columns.length; i += 1 ) {
								if ( columns[i].property !== null ) {
									if ( _NRLOrderIt.isDate(columns[i].type) ) {
										jobOrderNumbers[rowCount][columns[i].property] = _SQL.convertIntegerToDate(row.getResultByName(columns[i].name));
									}
									else {
										jobOrderNumbers[rowCount][columns[i].property] = row.getResultByName(columns[i].name);	
									}
								}
							}
					...
					handleCompletion : function (aReason) {
						...
						if ( aReason === Components.interfaces.mozIStorageStatementCallback.REASON_FINISHED ) {
							...
							if ( typeof(aCallback) !== 'undefined') {
								aCallback.completed(jobOrderNumbers);
							}
						}
						else {
							_logger.warn("Query canceled or aborted: " + sql);
