...
/**
 * The <code>JobOrderNumber.Tree</code> module.
 */
mil.navy.nrl.NRLOrderIt.JobOrderNumber.Tree = (function () {
	
	/*
	 * Private variables.
	 */
	
	var _logger = Log4Moz.repository.getLogger("mil.navy.nrl.NRLOrderIt.JobOrderNumber.Tree");
	...
	return {
		...
		get ID() {
			return _NRLOrderIt.JobOrderNumber.NAMESPACE + _NRLOrderIt.NAMESPACE_SEPARATOR + 'Tree'; 
		},
		...
		/**
		 * Fired when loading of this tree is complete.
		 */
		get LOAD_EVENT() {
		...
		/**
		 * Fired to edit an existing job order number.
		 */
		get EDIT_EVENT() {
		...
		get COLUMNS() {
			let columns = [
			               ...
			               {
			            	   id : this.ACCOUNT_NUMBER_COLUMN_ID,
		 ...
		 * is only sent if a job order number is selected in this tree.
		 */
		edit : function () {
		...
		/**
		 * Gets the selected job order number in the tree. Will return <code>null</code> 
		 * if no job order number is selected in the tree.
		 * 
		...
		/**
		 * Loads the tree with the job order numbers from the data source.
		 */
		load : function () {
			...
			_storage.retrieveAll({
				completed : function (results) {
					_jobOrderNumbers = results;
					_tree.view = _NRLOrderIt.JobOrderNumber.Tree;
				}
			});
		...
		getCellText : function (row, column) {
	    	if ( _jobOrderNumbers.length > 0 ) {	    		
	    		for ( let i = 0; i < this.COLUMNS.length; i += 1 ) {
	    			if ( column.id === this.COLUMNS[i].id ) {
	    				let columnObject = _jobOrderNumbers[row][this.COLUMNS[i].property];
	    ...
	    get rowCount() {			
	    	return _jobOrderNumbers.length;
	    }
	}
})();